...
def post_appointment(request):
    ...
    try:
        ...
        so_user = User.objects.filter(position= "Security officer")
        context["departments"] = departments   
        context["ld"] = oic
        context["so_users"] = so_user
        if request.method=="POST":
            ...
            if date_object < now_without_ms:
                messages.warning(request, "Please select the future time")
                return redirect("/home/postapntent")       
            oic  = User.objects.filter(id=oic).first()        
            so = User.objects.filter(id=so).first()
            print(oic)
            print(so)
            obj = Appointment(ld=oic, description=description,items =items, duration= duration, clearance_level=level,security_officer = so,   date=date_object)
        ...
        try:
            try:
                if request.user.position == postions[3][1]:
                    objs = Appointment.objects.filter(Q(r_user__gh_dh_user = request.user) & Q(clearance_level="brown")).order_by("-created_at")
                elif request.user.position == postions[2][1]:
                    print("true")
                    objs = Appointment.objects.filter(Q(r_user__tech_dir_user = request.user) & Q(date__gte = datetime.now())) & Appointment.objects.filter(Q(clearance_level="brown") | Q(clearance_level="green")).order_by("-created_at")
                    print(objs)
               
                elif request.user.position == postions[0][1]:
                    objs = Appointment.objects.filter(date__gte = datetime.now()).order_by("-created_at")
                elif request.user.position == postions[1][1]:
                    print("hello associate director")
                    objs = Appointment.objects.filter(Q(date__gte = datetime.now())).exclude(clearance_level = "red").order_by("-created_at")
            except Exception as e:
                pass  
...
def security_officer(request):
    ...
    print(today_date)
    try:
        objs = Appointment.objects.filter(security_officer = request.user , send_security = True)
        print("security objs")
        print(objs)
...
def forgot_password(request):
    if request.method == "POST":
        ...
        try:
            user = User.objects.get(pis=uidd)
        except Exception as e:
            messages.error(request, "PIS does not match. Please check again")
...
def close_clearance_reason(request,id):
    ...
    if request.method == "POST":
        ...
        print("unautorized person")
        print(close_clearance_reason)
        obj.reason_cancelation = close_clearance_reason
        obj.close_clearance = True
        obj.save()
...
def show_full_request_page(request,id):
    try:
        obj= Appointment.objects.get(id=id)
        context = {"obj": obj}

        if "approve" in request.POST:
            obj.final_status = "Approved"
            obj.save()
            return redirect("/home/shsfsdfow-readfafquest")

            
        if "reject" in request.POST:
            get_id = request.POST.get("reject")
            ...
            return redirect(f"/home/cfhadf-regfsa/{get_id}")
        if "dircancel" in request.POST:
            get_id = request.POST.get("reject")
            return redirect(f"/home/cfhadf-regfsa/{get_id}")
        ...
        print(e)
    return render(request, "full_show_request.html",context)